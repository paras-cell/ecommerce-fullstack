...
export const CartProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      // Keep cart in localStorage but don't sync with state if not authenticated
      // This allows persistence but warns user
    } else {
      // User is authenticated - load their cart from localStorage
      const stored = localStorage.getItem("cart");
      if (stored) {
        try {
