...
const Header = () => {
  ...
  const { cart, setCart } = useContext(CartContext) || { cart: [], setCart: () => {} };
  ...
  // Handle profile dropdown hover leave
      ...
      <div className="container">
        ...
        <div className="main-content">
          ...
          <div
            ...
            onMouseEnter={() => {
              if (isAuthenticated) setShowProfileDropdown(true);
            }}
            onMouseLeave={() => {
              if (isAuthenticated) setShowProfileDropdown(false);
            }}
          >
            {isAuthenticated ? (
              <>
                <a
                  ...
                  onMouseEnter={() => {
                    if (logreff.current && logreff.current.HoverEffect) logreff.current.HoverEffect();
                    handleHover("login", true);
                  }}
                  onMouseLeave={() => {
                    if (logreff.current && logreff.current.HoverEffectleave) logreff.current.HoverEffectleave();
                ...
                >
                  ...
                  ></span>
                  <p>{user?.fullName || user?.username || "User"}</p>
                </a>

                {showProfileDropdown && (
                  <div className="profile-dropdown" ref={profileDropdownRef} onMouseEnter={() => setShowProfileDropdown(true)} onMouseLeave={() => setShowProfileDropdown(false)}>
                    ...
                    <div className="dropdown-menu">
                      <button onClick={() => { navigate("/showdaddress"); setShowProfileDropdown(false); }}>
                        📍 My Addresses
                      </button>
                      <button onClick={() => { navigate("/order-history"); setShowProfileDropdown(false); }}>
                        📦 Order History
                      </button>
              ...
              <Login ref={logreff} onHover={(isHovered) => handleHover("login", isHovered)} />
            )}
          </div>
